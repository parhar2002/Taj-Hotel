...
{
    public partial class feedmanage : System.Web.UI.Page
    {
        ...
        {
            SqlCommand cmd = new SqlCommand(@"DELETE FROM [dbo].[Feedback]  WHERE [ID]='" + e.CommandArgument.ToString() + "'", con);
            con.Open();
            cmd.ExecuteNonQuery();
            Response.Redirect("feedmanage.aspx");
            con.Close();
        }
    }
}